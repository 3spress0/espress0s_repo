...
describe('provider resolution', () => {
  ...
  it('does not let a tgpt-era key silently retarget the calls at Google', async () => {
    ...
    const legacy = await decide({ apiKey: 'sk-proj-not-a-real-key-0123', apiKeySource: 'TGPT_API_KEY' },
      { tgptAvailable: true });
    ...
    assert.equal((await decide({ apiKey: 'sk-proj-not-a-real-key-0123', apiKeySource: 'AI_API_KEY' },
      { tgptAvailable: true })).provider, 'gemini');

    // Forced gemini with only a tgpt key: obey, but say so.
    const forced = await decide({ provider: 'gemini', apiKey: 'sk-proj-not-a-real-key-0123', apiKeySource: 'TGPT_API_KEY' });
